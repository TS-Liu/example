...
class NMTLossCompute(LossComputeBase):
    ...
    def _compute_loss(self, batch, output, output2, target_unk, target):
        ...
        tgt_unk_mask = Variable(target_unk.data.eq(self.unk_idx).float().unsqueeze(1)).contiguous().repeat(1, vocab_size_big, 1).transpose(1,2).view(-1,vocab_size_big)  ########
        tgt_no_unk_mask = Variable(target_unk.data.ne(self.unk_idx).float().unsqueeze(1)).contiguous().repeat(1, vocab_size, 1).transpose(1,2).view(-1,vocab_size)
        ...
        loss_data = loss.data.clone()
        LOSS =loss+loss_unk
        stats = self._stats(loss_data_unk, loss_data, scores_unk.data, scores.data, target_unk.view(-1).data*tgt_no_unk_mask.data, target.view(-1).data*tgt_unk_mask.data)
